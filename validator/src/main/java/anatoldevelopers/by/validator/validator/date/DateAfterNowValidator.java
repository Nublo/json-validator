...
public class DateAfterNowValidator extends AbstractValidator {
    ...
    public List<ValidationError> validate(Field field, Object fieldValue) {
        ...
        if (Type.DATE != field.getType()) {
            ValidationError error =
                    new ValidationError(field, "Invalid type for DateAfterNowValidator validator. Should be Date.");
            return Collections.singletonList(error);
        }

        final Date date = (Date) fieldValue;
        if (fieldValue != null && !date.after(new Date())) {
